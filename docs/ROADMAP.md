...
- **MTProto on prod** — the secrets directory is still empty on the live
  host; needs a dedicated account + `api_id`/`api_hash`. Deferred until
  the bot-backend path has been exercised more.
- README benchmark table records one bot-backend run; re-run after
  switching backends or chunk size.
...
- Dedicated MTProto account + `api_id`/`api_hash` — needed for the F5 golden test and a real MTProto deploy.
