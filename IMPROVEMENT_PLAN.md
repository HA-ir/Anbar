...
> **Last update:** 2026-09-02 17:41 (Asia/Tehran)
...
> **Baseline:** v0.15.19 @ c00fc76 — 260 tests passing. الان: 394 tests passing.
...
| PERF-01 | Range → دانلود کامل chunk از تلگرام | عملکرد | `src/anbar/api/download.py:_fetch_chunk_bytes` | کلیک وسط ویدیوی بزرگ = واکشی کل chunk 16MB از Telegram CDN برای پخش چند ثانیه. | micro-cache کوتاه‌عمر آخرین chunk (برای seeking)؛ بلندمدت: chunkهای کوچک‌تر برای مدیا. | 🔵 بلندمدت | ✅ Done (v0.15.25) |
...
|--------------------|----|--------|-------------|------------|
| 2026-09-02 19:10 | FEAT-SUBS | زیرنویس ویدئو (v0.15.39 @ 78f3901). ماژول `src/anbar/subtitles.py`: هر ترک در kv `subs:{obj_id}` (SRT→VTT خودکار، sanitize متن cue — تگ‌های whitelist VTT می‌مانند، بقیه escape؛ سقف ۲MB/۱۶ ترک). API عمومی `GET /f/{id}/subs` + `GET /f/{id}/subs/{tid}` (همان auth matrix مدیا: کوکی/bearer/سیگنیچر/پسورد). API ادمین: `GET/POST /api/v1/admin/objects/{id}/subs` + `PATCH/DELETE .../subs/{tid}` (لیبل/زبان/پیش‌فرض) + audit log. `_purge_object_blobs` ترک‌ها را با آبجکت حذف می‌کند. UI: نوار زیرنویس در مودال فایل (افزودن فایل SRT/VTT، کلیک=پیش‌فرض، دابل‌کلیک=تغییر نام، ✕=حذف) + `<track>`های native روی پلیر (منوی CC مرورگر). i18n فارسی/انگلیسی. | ۸ تست جدید `tests/test_subtitles.py` (تبدیل SRT→VTT + sanitize XSS، meta نام‌فایل، lifecycle کامل CRUD، جابجایی default، سقف ترک‌ها، auth 401/403/404، حذف با purge)؛ کل ۳۹۴ سبز؛ node --check پاس | ترک‌ها متادیتای ویدئو هستند نه آبجکت جدا — در لیست فایل‌ها ظاهر نمی‌شوند، کپی/انتقال folder جدا نیست. copy_object ترک‌ها را کپی نمی‌کند (کپی همان blobs است — ترک نسخه‌ی مستقل). صفحه آلبوم عمومی فعلاً زیرنویس نمی‌دهد (فاز بعد). |
| 2026-09-02 17:41 | DEP-SUBS | دیپلوی v0.15.39 روی پرود (anbar:prod @ febe419) + SMOKE PASSED + E2E پرود (loopback + dl.amiri-dev.ir) سبز | smoke ✅، E2E UI/آلبوم/ترک ✅ | آبجکت و آلبوم‌های تستی purge/revoke شدند |
...
| 2026-08-31 23:17 | PERF-01 | micro-cache chunk برای seeking (آخرین آیتم پلن). `ChunkMicroCache` جدید در `src/anbar/cache.py`: LRU درون-RAM keyed by (obj_id, chunk_index)، سقف بایت + TTL 120s، صفر نوشتن دیسک (zero-retention دست‌نخورده)، آمار hits/misses. در `download.py` دور `_fetch_chunk_bytes` با cache_key=(obj_id, idx) — seek تکراری داخل همان chunk دیگر کل 16MB از تلگرام نمی‌گیرد. knob رانتایم `seek_cache_mb` (0=خاموش، پیش‌فرض 32) در runtime SPEC + `_env_defaults` + `_sync_chunk_cache` (تغییر زنده بدون restart). `remove_object` در مسیر trash/purge. آمار `chunk_cache` در `/admin/status`. | ۱۶ تست جدید `tests/test_seek_cache.py` (unit: roundtrip/miss/oversized/LRU/TTL/replace/remove_object/close + E2E: seek تکراری → open_calls ثابت، بایت دقیق، status، override=0 → خاموش، delete → purge)؛ کل 348 سبز؛ ruff تمیز روی فایل‌های تغییر یافته | تست E2E کلیدی: دو seek متوالی در یک chunk → backend.open_calls فقط +1 (قبلاً +2). همچنین ۱۶ خطای pre-existing ruff در فایل‌های خارج از این تغییر (scripts/bench*، upload.py، db.py و…) مستند شد — دست نخوردند. |
