...
> **Last update:** 2026-08-31 14:10 (Asia/Tehran)
> **Status:** 17 از 21 مورد انجام شد — Batch 1-4 تمام ✅ (v0.15.20). باقیمانده: ARCH-01، ARCH-02، PERF-03، QUAL-01، QUAL-02 (حجیمی/معماری).
> **Baseline:** v0.15.19 @ c00fc76 — 260 tests passing. الان: 284 tests passing.
...
| ARCH-01 | آپلود تک‌توکنی روی BotPool | معماری | `src/anbar/main.py:67` (`app.state.bot_client = bot_pool.primary`) | BotPool برای دانلود round-robin می‌کند ولی آپلود همیشه از توکن اول می‌رود؛ سقف واقعی ~5.2MB/s (بنچمارک README). | توزیع chunk ها بین بک‌اندهای pool در `_store_stream`/`on_chunk` (هر chunk از backend بعدی). دقت: manifest باید backend هر chunk را نگه دارد (ستون backend فعلاً سراسری است → نیازمند طراحی). | 🔵 بلندمدت | ⬜ Not started |
| ARCH-02 | تک‌پروسه‌ای / SPOF پروسه | معماری | کل استک (uvicorn worker=1) | عملیات سنگین (backup، rebuild کانال، ZIP) event-loop را بلاک می‌کند؛ worker اضافه ممکن نیست چون pacing و harvester per-process هستند. | جدا کردن job های سنگین روی thread/process (zipper همین الگو را دارد)؛ طولانی‌مدت: صف job داخلی. | 🔵 بلندمدت | ⬜ Not started |
...
| QUAL-01 | rollback chunking دوبار تکرار شده | کیفیت کد | `api/upload.py:_store_stream` و `api/ingest.py:_run_job` | بلوک rollback و commit عیناً copy-paste؛ هر fix باید دو جا اعمال شود (ریسک drift). | استخراج `ObjectService.store_stream()` مشترک؛ هر دو route روی آن. | 🟡 متوسط | ⬜ Not started |
| QUAL-02 | miniapp احراز هویت واقعی ندارد | امنیت | `src/anbar/ui/miniapp.html:73,131` + `auth.py:verify_telegram_init_data` | تابع اعتبارسنجی initData نوشته و تست شده ولی به هیچ endpoint وصل نیست؛ miniapp همیشه 401 می‌گیرد و عملاً غیرقابل استفاده در auth=on. | endpoint `/ui/miniapp/session` که initData را validate و سشن صادر کند؛ miniapp از آن استفاده کند. | 🟡 متوسط | ⬜ Not started |
