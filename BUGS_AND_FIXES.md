...
> فایل تجمیعی باگ‌ها و فیکس‌ها. آخرین به‌روزرسانی: 2026-09-01 — نسخه v0.15.29

## v0.15.29 — 2026-09-01 (هات‌فیکس پریویو/دانلود — بازگرداندن پذیرش کلید ادمین در ?k=)

### FIX-05 · «هنوز هیچ پریویویی نیست و هیچ فایلی دانلود نمی‌شود» — Severity: Critical — ✅ ریشه‌ای فیکس شد
- **باس:** حتی بعد از فیکس v0.15.28 (silent re-login)، URLهای مستقیم مثل `https://dl.amiri-dev.ir/f/kDjYsQ5EHR2s?k=<کلید ادمین>` 401 می‌دادند. ریشه: **SEC-02 در v0.15.20** پذیرش کلید ادمین را از `?k=` حذف کرده بود (به‌بهانه‌ی leak در لاگ/referrer) — در حالی که UI همه‌ی پریویوها (`<img>/<video>/<audio>/<iframe>`، thumb) و دکمه‌ی دانلود را با همین شکل می‌سازد. نتیجه: بعد از انقضای کوکی سشن، همه‌چیز 401.
- **تصمیم مالک (حسین):** بازگشت رفتار قبل از v0.15.20 — کلید ادمین در `?k=` دوباره مجاز است. (این کلید خودِ مالک است و سمت سرور همه‌چیز را از قبل مجاز می‌کند؛ لینک‌های عمومی همچنان sig/exp هستند.)
- **فیکس (api/download.py · `_authenticate_download`):** `k` با admin_key یا api_key یا dynamic key مقایسه می‌شود؛ هر سه → مجوز دانلود. thumb هم از همین گارد استفاده می‌کند.
- **Verify (E2E واقعی):**
  - لینک دقیق کاربر → **HTTP 200 (75 bytes)**، سه بار متوالی ✓
  - مرورگر بدون هیچ کوکی‌ای: صفحه‌ی دانلود مستقیم رندر شد، `<img>` پیکسل گرفت (naturalHeight=8) ✓
  - در UI بدون سشن تازه: آپلود فایل جدید → پریویو متن (محتوای درست با highlight) ✓ → دکمه دانلود → 200 ✓
  - تست SEC-02 قدیمی معکوس شد (`test_admin_key_in_query_string_accepted` → 200)؛ کل سوئیت **364 passed** ✓
