...
### BUG-SUBS-1 · نوار زیرنویس خالی رندر می‌شد (shadowing)
- **علت:** در `renderSubsStrip` و `applyFmSubTracks` متغیر حلقه `const t of fmSubs` تابع ترجمه‌ی جهانی `t()` را می‌پوشاند → `t("subAdd")` کرش می‌کرد و strip خالی می‌ماند.
- **فیکس:** تغییر نام متغیر حلقه به `trk` در هر دو تابع.
- **راستی‌آزمایی:** E2E مرورگر واقعی — chips رندر شدند.

### BUG-SUBS-2 · نوار زیرنویس کنار ویدئو می‌نشست (flex row)
- **علت:** `.fm-preview` افقی flex است؛ strip به‌جای زیر ویدئو کنارش می‌رفت.
- **فیکس:** کلاس `.fm-preview.video-col` (flex-direction:column) روی preview ویدئو اعمال شد.

### BUG-SUBS-3 · lightbox آلبوم زیرنویس نداشت
- **فیکس:** PAYLOAD صفحه‌ی آلبوم هر آیتم ویدئویی `subs:[{id,label,lang,default}]` می‌گیرد؛ `openLb` برای ویدئو `<track>`های سیم‌گاارد با همان sig لینک می‌سازد و mode پیش‌فرض را ست می‌کند. بدون subs_meta، رفتار قبلی.
...
## v0.15.42 — فیکس زیرنویس ویدئوی اشتباه + UX استخراج
- **باگ (کاربر گزارش کرد):** بعد از جابه‌جایی بین ویدئوها، زیرنویس ویدئوی قبلی به ویدئوی جدید وصل می‌شد. علت: `applyFmSubTracks` هنگام `loadedmetadata` با `fmSubs` قدیمی اجرا می‌شد و وقتی لیست تازه می‌رسید دوباره attach نمی‌شد. فیکس: توکن `fmSubsSeq` (پاسخ قدیمی دور ریخته می‌شود) + رد ترک‌های cross-object + attach مجدد بعد از رسیدن لیست.
- **UX (کاربر گزارش کرد):** دکمه «⤓ از داخل ویدئو» حین کار هیچ بازخوردی نداشت. الان: اسپینر + «در حال استخراج…»، دابل‌کلیک نادیده، پایان/خطا همیشه دکمه را برمی‌گرداند + toast نتیجه.
- تنظیمات: جمله EBML/دیسک موقت از متن Cloudflare حذف شد (طبق درخواست کاربر).
- نکته دیسک: استخراج موقتاً به اندازه‌ی ویدئو در `data/tmp` فایل می‌نویسد و همان لحظه پاک می‌کند؛ خود VTT در kv دیتابیس است.

## v0.15.43 — پیام استخراج تکراری، مخفی‌شدن دکمه وقتی قابلیت خاموش است، toast راست‌چین
- **باگ (کاربر):** استخراج دوباره‌ی زیرنویسِ ویدئویی که قبلاً استخراج شده پیام «زیرنویس تعبیه‌شده‌ای پیدا نشد» می‌داد. علت: همه‌ی ترک‌ها به‌خاطر dedupe حذف می‌شدند و `added=[]` با حالت «زیرنویس ندارد» یکسان گزارش می‌شد. فیکس: اندپوینت `already_imported:true` برمی‌گرداند و UI پیام «زیرنویس این ویدئو قبلاً استخراج شده است» نشان می‌دهد.
- **باگ (کاربر):** دکمه «⤓ از داخل ویدئو» با سوییچ خاموش هم دیده می‌شد و کلیک روی آن ارور 409 انگلیسی می‌داد. فیکس: وقتی `subs_extract_enabled=0` است دکمه اصلاً رندر نمی‌شود.
- **باگ (کاربر):** toast از کادر بیرون می‌زد و در زبان فارسی چپ‌چین بود. فیکس: `direction:rtl` برای toastها در حالت فارسی + `overflow:hidden` + `max-width:92vw` با سه‌نقطه.

## v0.15.44 / BUG-19 — فیکس پرپر زدن پیش‌نمایش در ناوبری پوشه‌ها و نمایش/پخش پیش‌نمایش ویدیوها و فایل‌ها
- **باگ ۱ (پرپر زدن پیش‌نمایش هنگام ورود به پوشه و بازگشت به ریشه):** هنگام جابه‌جایی بین پوشه‌ها و بازگشت، المنت `.gallery` به‌طور کامل از DOM حذف و مجدداً ایجاد می‌شد که باعث می‌شد استایل موقتاً به حالت جدول برگردد و هدرهای جدول فلش بزنند. همچنین `loading="lazy"` روی کارت‌های گالری باعث پرش و لود با تاخیر می‌شد.
  - **فیکس:** اضافه شدن کلاس دائمی `.view-gallery` به کانتینر `#tblWrap` و نگه داشتن کانتینر `.gallery` به جای حذف و ساخت مداوم، حذف `loading="lazy"` از کارت‌های گالری برای جلوگیری از لایوت‌شیفت و چشمک زدن.
- **باگ ۲ (عدم نمایش/پخش درست پیش‌نمایش ویدیوها و فایل‌ها هنگام کلیک روی آن‌ها):**
  - در نمای گالری، در لیسنر کلیک کارت `e.target.closest(".gacts,.selbox,a,button,audio,video")` وجود داشت که باعث می‌شد با کلیک روی تامبنیل ویدیو ایونت نادیده گرفته شود و پاپ‌آپ پیش‌نمایش باز نشود.
  - **فیکس:** حذف سلکتور `video` از شرط انصراف کلیک، افزودن `pointer-events: none` و `#t=0.001` به المنت ویدیوی کارت گالری برای نمایش فریم اولیه و اطمینان از کلیک‌پذیری مستقیم. افزودن هندلر خطای پخش در مودال ویدیو و تنظیم هدرهای اعتبارسنجی در پیش‌نمایش متن.
- **تست:** تست‌های E2E جدید با Playwright (`tests/test_e2e_playwright.py`) برای پایداری کروم گالری حین ناوبری پوشه‌ها و کلیک و پخش ویدیوی پیش‌نمایش.
