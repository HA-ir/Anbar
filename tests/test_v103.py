...
def _upload(client, content=b"manage-me"):
    r = client.post("/api/v1/upload", headers=ADMIN,
                    files={"file": ("doc.txt", io.BytesIO(content),
...
def test_manage_page_renders_settings(client):
    oid = _upload(client)
    m = client.post(f"/f/{oid}/link?ttl=3600&password=pp1&max_dl=3",
                    headers=ADMIN).json()
...
def test_manage_page_unknown_link_404(client):
    r = client.get("/api/v1/admin/links/nope12345678/manage?exp=123",
                   headers=ADMIN)
...
def test_manage_flow_revoke_then_remint(client):
    ...
    oid = _upload(client)
    m = client.post(f"/f/{oid}/link?ttl=3600&slug=mng&password=old",
                    headers=ADMIN).json()
    exp = int(m["expires_at"])
    # 1. revoke old
    ...
    nm = client.post(f"/f/{oid}/link?ttl=604800&slug=mng&password=new&max_dl=5",
                     headers=ADMIN)
...
def test_browser_correct_pw_no_loop(client):
    ...
    url = client.post(f"/f/{oid}/link?ttl=600&password=r4z",
                      headers=ADMIN).json()["url"]
...
def test_slug_journey_via_browser(client):
    ...
    client.post(f"/f/{oid}/link?ttl=600&password=k3&slug=journey103",
                headers=ADMIN)
