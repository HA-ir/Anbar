...
def test_verify_telegram_init_data():
    ...
    params = {
        ...
        "user": '{"id":279058397,"first_name":"Vladislav","last_name":"K","username":"vladislav_k","language_code":"en"}',
    }
    data_check_string = "\n".join(f"{k}={v}" for k, v in sorted(params.items()))
    secret_key = hmac.new(b"WebAppData", bot_token.encode(), hashlib.sha256).digest()
    valid_hash = hmac.new(secret_key, data_check_string.encode(), hashlib.sha256).hexdigest()

    init_data = f"auth_date={params['auth_date']}&query_id={params['query_id']}&user={params['user']}&hash={valid_hash}"

    # 1. Valid signature passes
    assert verify_telegram_init_data(init_data, bot_token) is True

    # 2. Tampered data fails
    tampered = f"auth_date={params['auth_date']}&query_id=hacked&user={params['user']}&hash={valid_hash}"
    ...
    old_init_data = f"auth_date={old_params['auth_date']}&query_id={old_params['query_id']}&user={old_params['user']}&hash={old_hash}"
