...
def test_revoke_kills_link(client):
    ...
    assert client.post(f"/api/v1/admin/links/{oid}/revoke/{exp}", headers=ADMIN).status_code == 404
...
def test_soft_delete_hides_then_restore(client):
    ...
    oid = o["id"]
    # visible in list
    ids = [x["id"] for x in client.get("/api/v1/admin/objects", headers=ADMIN).json()["objects"]]
    assert oid in ids
    # delete → trashed
...
def test_purge_destroys_for_real(client):
    ...
    assert client.post(f"/api/v1/admin/trash/{oid}/restore", headers=ADMIN).status_code == 404
